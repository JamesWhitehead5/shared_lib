...
class RCWA:
    ...
    class Simulation:
        ...
        def run(self):
            ...
            if FIELD==None:
                Ex = e[:,:,0]  # adding 1 removes incident wave 
                Ey = e[:,:,1]  # adding 1 removes incident wave 
                Ez = e[:,:,2]  # adding 1 removes incident wave 
            else:                
                (a,b,_) = np.shape(e)
                edge = (1-FIELD)/2.0
                
                Ex = e[:,:,0]
                Ex = Ex[int(a*edge):int(a*(1-edge)),int(b*edge):int(b*(1-edge)),0]
                Ey = e[:,:,1]
                Ey = Ex[int(a*edge):int(a*(1-edge)),int(b*edge):int(b*(1-edge)),0]
                Ez = e[:,:,2]
                Ez = Ex[int(a*edge):int(a*(1-edge)),int(b*edge):int(b*(1-edge)),0]
                
            
            return pd.Series([(Ex.tolist()), (Ey.tolist()), (Ez.tolist())])